...
export default async function DashboardPage() {
  ...
  const currentPlan: "premium" | "max" | null = isSubscribed || isCanceling
    ? isMaxPlan ? "max" : "premium"
    : null;

  const currentBilling: "monthly" | "yearly" | null = isSubscribed || isCanceling
    ? (priceId === maxYearlyId || priceId === premiumYearlyId) ? "yearly" : "monthly"
    : null;
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-zinc-50 dark:bg-black">
      ...
      <aside className="flex w-64 shrink-0 flex-col border-r border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Bonjour, {session.user.name}</p>
        </div>

        <div className="mb-6 rounded-lg bg-zinc-50 p-3 dark:bg-zinc-900">
          <p className="text-xs text-zinc-500 dark:text-zinc-400">Email</p>
          <p className="mt-0.5 truncate text-sm font-medium text-zinc-900 dark:text-zinc-50">{session.user.email}</p>
        </div>
      ...
      <main className="flex flex-1 flex-col overflow-y-auto p-8">
        <h2 className="mb-6 text-xl font-semibold text-zinc-900 dark:text-zinc-50">Abonnement</h2>
        <SubscriptionButton
