  ...
  return (
    <div className="w-full rounded-xl border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-950">
      <p className="mb-3 text-sm text-red-700 dark:text-red-300">
        Confirmer l&apos;annulation ? Votre accès restera actif jusqu&apos;à la
        fin de la période.
      </p>
      <div className="flex gap-2">
        <Button
            ...
            {upgrades.map(({ tier, billing }) => (
              <PlanCard
                ...
                actionLabel={
                  loading
                    ? "Calcul en cours..."
                    : `Upgrade — ${PLANS[tier][billing].price} / mois`
                }
              />
            ))}
          </div>
...
export function SubscriptionButton({
  ...
  const handleUpgradePreview = async (tier: Tier, billing: Billing) => {
    ...
    try {
      const res = await fetch(
        `/api/stripe/upgrade?tier=${tier}&billing=${billing}`,
        {
          method: "POST",
        },
      );
  ...
  return <NewSubscriptionView loading={loading} onCheckout={handleCheckout} />;
}
